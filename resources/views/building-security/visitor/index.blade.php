        ...
        <table class="min-w-full shadow-lg rounded-lg overflow-hidden">
            <thead>
                <tr>
                    ...
                    <th class="font-bold py-2 px-8">Contact Details</th>
            ...
            <tbody>
                @foreach ($security_data->sortByDesc('created_at') as $key => $security)
                    <tr>
                        ...
                        <td class="py-2 px-8">
                            @if ($security->status_of_visitor === 0 && $security->card_id == null)
                            <form class="flex flex-col gap-2 items-center" action="{{ route('building-security.visitor-card-assign', $security->id) }}"
                                method="post">
                                @csrf
                                @method('PATCH')
                                <div class="w-32">
                                    <select class="block w-full p-2 text-sm rounded-lg" id="card_id" name="card_id" class="form-input" required>
                                        <option value="">Select Card</option>
                                        @foreach ($cards as $card)
                                        <option value="{{ $card->id }}">{{ $card->serial_id }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <button type="submit"
                            ...
                            @else
                                {{ $security->card?->serial_id ?? "N/A" }}
                            @endif
                        </td>
                        <td>
                            @php
                                $statusColor = match($security->card_status) {
                                    'Pending' => 'text-warning',
                                    'Card Assigned' => 'text-primary',
                                    'Card Scanned at wrong place' => 'text-danger',
                                    'Card Scanned at right place' => 'text-success',
                                    default => 'text-gray'
                                };
                            @endphp
                            <span class="{{ $statusColor }}">{{ $security->card_status ?? 'Pending' }}</span>
                        </td>
                        {{-- <td> --}}
                            {{-- @if (!empty($security->card->serial_id))
                                {{ $security->card->serial_id }}
                            @elseif($security->status_of_visitor === 0)
                                <form action="{{ route('buildings_security_visitor.assignCard') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="security_id" value="{{ $security->id }}">

                                    <label for="card_id"><strong>Card</strong></label>
                                    <select id="card_id" name="card_id" class="form-input" required onchange="this.form.submit()" style="
                                    width: 8rem;
                                ">
                                        <option value="">Select Card</option>
                                        @foreach ($cards as $item)
                                            <option value="{{ $item->id }}">{{ $item->serial_id }}</option>
                                        @endforeach
                                    </select>
                                </form>
                            @else
                        ...
                        <td class="py-2 px-8">Mobile:{{ $security->mobile }}
                        Whatsapp:{{ $security->whatsapp }}</td>
                            ...
                            <td class="py-2 px-8">
                                ...
                                <div class="container mt-5">
                                    <div x-data="{ open: false }">
                                        ...
                                        <button type="button" class="btn btn-sm btn-danger" @click="open = !open"
                                                        ...
                                                        <form action="{{ route('building-security.timeout-remark') }}"
                                                            ...
                                                            <div class="form-group">
                                                                ...
                                                                <div class="flex gap-2 mt-5">
                                                                    <input id="card_return" type="checkbox"
                                                                        name="card_return"
                                                                        placeholder="Enter your remark"
                                                                        class="" value="1" />
                                                                    <label
                                                                        for="card_return"><strong>Card Returned</strong></label>
                                                                </div>
                                                            </div>
                                                            <br>
                                                            {{-- <input type="hidden" name="card_returned" value="0"> --}}

        {{-- <div class="form-group mb-4">
            <label class="inline-flex items-center">
                <input type="checkbox" name="card_returned" value="1" class="form-checkbox" >
                <span class="ml-2"><strong>Please Check If Card Returned</strong></span>
            </label>
        </div> --}}
                                            ...
                                            <div x-show="open" x-transition x-transition.duration.300
                                                ...
                                                <div class="p-5">
                                                    @php
                                                        ...
                                                        $visitor = App\Models\Visitor_Master::with('card')->where(
                                                            'visitor_id_detected',
                                                            $visitorId,
                                                        )->get();
                                                    @endphp
                        ...
                        <td>
                            ...
                            @else
                                Block
                                @if ($security->tenant_block == 1)
                                <form action="{{route('unblock-security')}}" method="POST">
                                    @csrf
                                    <input type="hidden" name="unblockvisitor" value="{{$security->visitor_id}}"/>
                                    <button
...
@endif -->
                        ...
                        {{-- <td>
                            @if (is_null($security->status_of_visitor) || $security->status_of_visitor === '')
                                ...
                                <button class="btn btn-danger btn-reject" data-id="{{ $security->id }}">Decline</button>
        ...
        <div id="customModal" class="custom-modal">
            <div class="custom-modal-content">
                ...
                <div id="rejectFields" class="remark-fields d-none">
                    <label for="rejectRemark">Decline Remark</label>
