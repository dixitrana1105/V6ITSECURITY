<x-layout.default>
    ...
    <div class="table-responsive">
        <table  id="visitorTable" class="min-w-full shadow-lg rounded-lg overflow-hidden">
            <thead>
                <tr>
                    ...
                    <th class="font-bold py-2 px-8">Card Return Status</th>
...
<tr>
            ...
            <td class="py-2 px-8">{{ $security->card?->serial_id ?? 'N/A' }}</td>
              <td class="py-2 px-8">
                            @php
                            ...
                            @endphp
                            <span class="{{ $statusColor }}">{{ $security->card_status ?? 'N/A' }}</span>
                        </td>
            <td class="py-2 px-8">
    @if ($security->card_returned === 1)
        Returned
    @elseif ($security->card_returned === 0)
        Not Returned
    @else
        <!-- Do nothing, or leave it empty -->
    @endif
</td>
...
<script>
    $(document).ready(function () {
        $('#visitorTable').DataTable({
            ...
            "lengthMenu": [5, 10, 25, 50], // Entries per page options
