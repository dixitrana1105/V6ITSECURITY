            ...
            <tbody>
                @foreach ($security_data as $key => $security)
                    <tr>
                        ...
                        <td class="py-2 px-8" hidden>{{ $security->id }}</td>
                        <td id="subTenantId" hidden>{{ $match_subenant_id->sub_tenant_id }}</td>
                                        ...
                                        <div class="fixed inset-0 bg-[black]/60 z-[999] hidden"
                                            ...
                                            <div class="flex items-start justify-center min-h-screen px-4"
                                                ...
                                                <div x-show="open" x-transition x-transition.duration.300
                                                    ...
                                                    <div class="p-5">
                                                        <form action="{{ route('building-tenant.timeout-remark') }}"
                        ...
                        <td class="py-2 px-8">
                            <form action="{{ route('building-tenant.visitor-status', $security->id) }}"
                                method="post">
                                @csrf
                                @method('PATCH')
                        ...
                        <td>
                            @if (is_null($security->status_of_visitor) || $security->status_of_visitor === '')
                                ...
                                <button class="btn btn-primary btn-prove"
                                    data-id="{{ $security->id }}">Approve</button>
                                <button class="btn btn-danger btn-reject"
                                    data-id="{{ $security->id }}">Decline</button>
                                <button class="btn btn-warning btn-resedual"
                                    data-id="{{ $security->id }}">Reschedule</button>
                            @else
        ...
        <div id="remarkModal" class="fixed inset-0 flex items-center justify-center hidden z-50">
            <div class="bg-white rounded-lg shadow-lg p-4 max-w-sm w-full relative">
                ...
                <div class="mt-4 flex justify-end space-x-2">
                    ...
                    <button id="closeModal" class="px-4 py-2 bg-blue-500 text-white rounded">Close</button>
                </div>
            </div>
        </div>
        ...
        <div id="customModal" class="custom-modal">
            <div class="custom-modal-content">
                ...
                <div id="rejectFields" class="remark-fields d-none">
                    ...
                    <input type="text" class="form-input" id="rejectRemark" placeholder="Enter decline remark">
                </div>
...
modalTitle.style.visibility = "hidden"; // hidden but keeps space
                ...
                document.querySelector('.btn-save').addEventListener('click', function() {
                    ...
                    var subTenantId = document.getElementById('subTenantId').innerText.trim();
                    ...
                    $.ajax({
                        url: '{{ route('tenant-save-remark') }}', // Your Laravel route to handle saving
                        type: 'POST',
                        data: dataToSend,
