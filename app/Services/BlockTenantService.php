...
class BlockTenantService
{
    public function blockTenant($request)
    {
        ...
        // Extract visitor ID
        $visitorId = 'TENA'.$request->visitor_id;
        

       $table_id = TenantVisitor::where('visitor_id', $visitorId)->value('id');

if (!$table_id) {
    $table_id = Visitor_Master::where('visitor_id', $request->visitor_id)->value('id');
}
        ...
        Visitor_Master::where('visitor_id', $request->visitor_id)->update(['tenant_block' => 1]);
        return ['success' => true, 'message' => 'Visitor blocked successfull.'];
    }
