  ...
  public function index_visitor_log(Request $request)
{
    ...
    // --- Base Queries ---
    $tenantQuery = TenantVisitor::where('building_id', $building_id)
        ->whereNotNull('out_time_remark');

    $visitorQuery = Visitor_Master::with('card')
        ->where('building_id', $building_id)
        ->whereNotNull('out_time_remark');

    // --- Filters ---
    if ($request->filled('dateFrom') && $request->filled('dateTo')) {
        $tenantQuery->whereBetween('date', [$request->dateFrom, $request->dateTo]);
        $visitorQuery->whereBetween('date', [$request->dateFrom, $request->dateTo]);
    }

    if ($request->filled('status') && $request->status !== 'all') {
        $status = $request->status === 'active' ? 1 : 0;
        $tenantQuery->where('status', $status);
        $visitorQuery->where('status', $status);
    }

    // --- Get Data ---
    $security_data = $tenantQuery->get();

    // If no records in TenantVisitor, fallback to Visitor_Master
    if ($security_data->isEmpty()) {
        $security_data = $visitorQuery->get();
    }

    // Always load tenant list (for dropdowns, etc.)
    $tenants = TenantVisitor::where('building_id', $building_id)->get();
