    ...
    public function visitor_card_assign(Request $request, $id)
{
    ...
    // Get tenant flat/office number
    $tenantFlatNo = $visitor->tenant_flat_office_no;

    // Find matching BuildingAdminTenant entry
    $buildingTenant = BuildingAdminTenant::where('flat_office_no', $tenantFlatNo)
                        ->whereNull('sub_tenant_id')
                        ->first();

    // If match found, update reader_id only
    if ($buildingTenant) {
        $visitor->reader_id = $buildingTenant->reader_id;
         $visitor->card_id = $request->card_id;
          $visitor->card_status = "Card Assigned";
        
        $visitor->save(); // Save only if reader_id is updated
    }

    // This part runs ALWAYS
    Card::where('id', $request->input('card_id'))->update(['assign_status' => "assigned"]);

    return redirect()->back()->with('success', 'Card assign status updated.');
}
    ...
    public function timeoutRemark(Request $request)
{
    ...
    if ($security) {
        ...
        $security->card_returned = $request->card_return;
        $security->out_time = Carbon::now('Asia/Kolkata')->format('H:i');
        $security->save();

// dd($request->card_return);
        if ($request->card_return == 1) {
    ...
    public function unblockSecurity(Request $request)
    {
        ...
        if ($visitor) {
            $visitor->update(['visitor_block' => null, 'tenant_block' => null]);
        }
